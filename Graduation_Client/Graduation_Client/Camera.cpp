...
{
	if (m_pcbMappedCamera == nullptr) return;
...
void CustomizingCamera::start(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList)
{
	...
	m_pPlayer = Input::GetInstance()->m_pPlayer;
	SetTimeLag(0.05f);
	SetOffset(XMFLOAT3(0.0f, 1.5f, 3.0f));
...
void PowerInteractionCamera::start(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList)
{
	Camera::start(pd3dDevice, pd3dCommandList);
	m_nMode = THIRD_PERSON_CAMERA;
	m_xmf3Up = XMFLOAT3(0.0f, 1.0f, 0.0f);
	...
	m_xmf3Look = Vector3::Normalize(m_xmf3Look);
	m_pPlayer = Input::GetInstance()->m_pPlayer;
	Power = m_pPlayer->m_pNearInteractionObejct;
	SetTimeLag(0.05f);
	SetOffset(XMFLOAT3(0.0f, 0.0f, -1.0f));
	GenerateProjectionMatrix(1.01f, 5.0f, ASPECT_RATIO, 40.0f);
	SetViewport(0, 0, FRAME_BUFFER_WIDTH, FRAME_BUFFER_HEIGHT, 0.0f, 1.0f);
	SetScissorRect(0, 0, FRAME_BUFFER_WIDTH, FRAME_BUFFER_HEIGHT);
	m_xmf3Position = Power->GetPosition();
	switch (Power->GetDIR()) {
	case 0:
		m_xmf3Position.z += 2.0f;
		break;
	case 1:
		m_xmf3Position.x += 2.0f;
		break;
	case 2:
		m_xmf3Position.z -= 2.0f;
		break;
	...
	SetLookAt(Power->GetPosition());
	/*if (m_pPlayer)
	{
		XMFLOAT4X4 xmf4x4Rotate = Matrix4x4::Identity();
		XMFLOAT3 xmf3Right = Power->GetRightVector();
		XMFLOAT3 xmf3Up = Power->GetUpVector();
		XMFLOAT3 xmf3Look = Power->GetLookVector();
		xmf4x4Rotate._11 = -xmf3Right.x; xmf4x4Rotate._21 = xmf3Up.x; xmf4x4Rotate._31 = -xmf3Look.x;
		xmf4x4Rotate._12 = -xmf3Right.y; xmf4x4Rotate._22 = xmf3Up.y; xmf4x4Rotate._32 = -xmf3Look.y;
		xmf4x4Rotate._13 = -xmf3Right.z; xmf4x4Rotate._23 = xmf3Up.z; xmf4x4Rotate._33 = -xmf3Look.z;

		XMFLOAT3 xmf3Offset = Vector3::TransformCoord(m_xmf3Offset, xmf4x4Rotate);
		XMFLOAT3 pos = Power->GetPosition();
		XMFLOAT3 xmf3Position = Vector3::Add(pos, xmf3Offset);
		XMFLOAT3 xmf3Direction = Vector3::Subtract(xmf3Position, m_xmf3Position);
		float fLength = Vector3::Length(xmf3Direction);
		xmf3Direction = Vector3::Normalize(xmf3Direction);
...
void PowerInteractionCamera::update(float elapsedTime)
{
	Camera::update(elapsedTime);
}

void PowerInteractionCamera::SetLookAt(XMFLOAT3& xmf3LookAt)
{
