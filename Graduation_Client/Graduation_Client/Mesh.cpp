		...
		else if (!strcmp(pstrToken, "<TextureCoords0>:"))
		{
			...
			if (nTextureCoords > 0)
			{
				...
				for (int i = 0; i < m_nVertices; ++i) {
					m_pxmf2TextureCoords0[i] = XMFLOAT2(0, 0);
					m_pd3dTextureCoord0Buffer = ::CreateBufferResource(pd3dDevice, pd3dCommandList, m_pxmf2TextureCoords0, sizeof(XMFLOAT2) * m_nVertices, D3D12_HEAP_TYPE_DEFAULT, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, &m_pd3dTextureCoord0UploadBuffer);
...
void SkinnedMesh::UpdateShaderVariables(ID3D12GraphicsCommandList* pd3dCommandList)
{
	...
	if (m_pd3dcbBoneTransforms)
	{
		...
		{
			XMStoreFloat4x4(&m_pcbxmf4x4BoneTransforms[j], XMMatrixTranspose(XMLoadFloat4x4(&m_ppSkinningBoneFrameCaches[j]->m_xmf4x4World)));
		}
	}
}
...
WallMesh::WallMesh(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList) : Mesh(pd3dDevice, pd3dCommandList)
{
}
...
void WallMesh::LoadMeshFromFile(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, char* pstrFileName)
{
	FILE* pFile = NULL;
	...
	nReads = (UINT)::fread(&m_xmf3AABBCenter, sizeof(float), 3, pFile);
	nReads = (UINT)::fread(&m_xmf3AABBExtents, sizeof(float), 3, pFile);
	...
	nReads = (UINT)::fread(&(m_pnSubSetIndices[0]), sizeof(int), 1, pFile);
	if (m_pnSubSetIndices[0] > 0)
	{
		m_ppnSubSetIndices[0] = new UINT[m_pnSubSetIndices[0]];
		nReads = (UINT)::fread(m_ppnSubSetIndices[0], sizeof(UINT), m_pnSubSetIndices[0], pFile);
...
SkyBoxMesh::SkyBoxMesh(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList) : Mesh(pd3dDevice, pd3dCommandList)
{
	m_nVertices = 36;
...
UIMesh::UIMesh(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, float x , float y , float width , float height ) : Mesh(pd3dDevice, pd3dCommandList)
{
	m_nVertices = 6;
	m_d3dPrimitiveTopology = D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
...
ParticleMesh::ParticleMesh(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, XMFLOAT3 xmf3Position, XMFLOAT3 xmf3Velocity, float fLifetime, XMFLOAT3 xmf3Acceleration, XMFLOAT3 xmf3Color, XMFLOAT2 xmf2Size, UINT nMaxParticles) : Mesh(pd3dDevice, pd3dCommandList)
{
...
void ParticleMesh::ReleaseUploadBuffers()
{
