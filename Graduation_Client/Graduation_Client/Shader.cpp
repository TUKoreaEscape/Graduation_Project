...
void Shader::CreateShader(ID3D12Device* pd3dDevice, ID3D12RootSignature* pd3dGraphicsRootSignature, D3D12_PRIMITIVE_TOPOLOGY_TYPE d3dPrimitiveTopology, UINT nRenderTargets, DXGI_FORMAT* pdxgiRtvFormats, DXGI_FORMAT dxgiDsvFormat)
{
	::ZeroMemory(&m_d3dPipelineStateDesc, sizeof(D3D12_GRAPHICS_PIPELINE_STATE_DESC));
...
{
	UINT ncbElementBytes = ((sizeof(VS_PS_DEBUG_OPTIONS) + 255) & ~255); //256의 배수
...
D3D12_INPUT_LAYOUT_DESC DoorUIShader::CreateInputLayout()
{
	UINT nInputElementDescs = 2;
	D3D12_INPUT_ELEMENT_DESC* pd3dInputElementDescs = new D3D12_INPUT_ELEMENT_DESC[nInputElementDescs];
...
D3D12_DEPTH_STENCIL_DESC DoorUIShader::CreateDepthStencilState()
{
	D3D12_DEPTH_STENCIL_DESC d3dDepthStencilDesc;
	d3dDepthStencilDesc.DepthEnable = FALSE;
	d3dDepthStencilDesc.DepthWriteMask = D3D12_DEPTH_WRITE_MASK_ZERO;
...
D3D12_BLEND_DESC DoorUIShader::CreateBlendState()
{
	D3D12_BLEND_DESC d3dBlendDesc;
	::ZeroMemory(&d3dBlendDesc, sizeof(D3D12_BLEND_DESC));
	d3dBlendDesc.AlphaToCoverageEnable = FALSE;
...
D3D12_RASTERIZER_DESC DoorUIShader::CreateRasterizerState()
{
	D3D12_RASTERIZER_DESC d3dRasterizerDesc;
	::ZeroMemory(&d3dRasterizerDesc, sizeof(D3D12_RASTERIZER_DESC));
	d3dRasterizerDesc.FillMode = D3D12_FILL_MODE_SOLID;
...
D3D12_SHADER_BYTECODE DoorUIShader::CreateVertexShader()
{
	return(Shader::CompileShaderFromFile(L"Shaders.hlsl", "VSDoorUI", "vs_5_1", &m_pd3dVertexShaderBlob));
}

D3D12_SHADER_BYTECODE DoorUIShader::CreatePixelShader()
{
	return(Shader::CompileShaderFromFile(L"Shaders.hlsl", "PSDoorUI", "ps_5_1", &m_pd3dPixelShaderBlob));
}

void DoorUIShader::CreateShader(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature)
{
	Shader::CreateShader(pd3dDevice, pd3dCommandList, pd3dGraphicsRootSignature);
...
D3D12_DEPTH_STENCIL_DESC MinimapShader::CreateDepthStencilState()
{
	D3D12_DEPTH_STENCIL_DESC d3dDepthStencilDesc;
	d3dDepthStencilDesc.DepthEnable = FALSE;
	d3dDepthStencilDesc.DepthWriteMask = D3D12_DEPTH_WRITE_MASK_ZERO;
...
D3D12_BLEND_DESC MinimapShader::CreateBlendState()
{
	D3D12_BLEND_DESC d3dBlendDesc;
	::ZeroMemory(&d3dBlendDesc, sizeof(D3D12_BLEND_DESC));
	d3dBlendDesc.AlphaToCoverageEnable = FALSE;
...
D3D12_RASTERIZER_DESC MinimapShader::CreateRasterizerState()
{
	D3D12_RASTERIZER_DESC d3dRasterizerDesc;
	::ZeroMemory(&d3dRasterizerDesc, sizeof(D3D12_RASTERIZER_DESC));
	d3dRasterizerDesc.FillMode = D3D12_FILL_MODE_SOLID;
...
{
	return(Shader::CompileShaderFromFile(L"Shaders.hlsl", "VSMinimapUI", "vs_5_1", &m_pd3dVertexShaderBlob));
}

D3D12_SHADER_BYTECODE MinimapShader::CreatePixelShader()
{
	return(Shader::CompileShaderFromFile(L"Shaders.hlsl", "PSMinimapUI", "ps_5_1", &m_pd3dPixelShaderBlob));
}

void MinimapShader::CreateShader(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature)
{
	Shader::CreateShader(pd3dDevice, pd3dCommandList, pd3dGraphicsRootSignature);
...
void MinimapShader::CreateShader(ID3D12Device* pd3dDevice, ID3D12RootSignature* pd3dGraphicsRootSignature, UINT nRenderTargets, DXGI_FORMAT* pdxgiRtvFormats, DXGI_FORMAT dxgiDsvFormat)
{
	Shader::CreateShader(pd3dDevice, pd3dGraphicsRootSignature, nRenderTargets, pdxgiRtvFormats, dxgiDsvFormat);
}
...
//}
//
//D3D12_SHADER_BYTECODE IlluminatedShader::CreateVertexShader(ID3DBlob** ppd3dShaderBlob)
//{
//	return(Shader::CompileShaderFromFile(L"Shaders.hlsl", "VSLighting", "vs_5_1", ppd3dShaderBlob));
//}
//
//D3D12_SHADER_BYTECODE IlluminatedShader::CreatePixelShader(ID3DBlob** ppd3dShaderBlob)
//{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
DepthRenderShader::DepthRenderShader(LIGHT* pLights,GameScene* scene)
{
	m_pScene = scene;
	m_pLights = pLights;
...
DepthRenderShader::~DepthRenderShader()
{
	if (m_pToLightSpaces) delete m_pToLightSpaces;
}

D3D12_SHADER_BYTECODE DepthRenderShader::CreatePixelShader()
{
	return(Shader::CompileShaderFromFile(L"Shaders.hlsl", "PSDepthWriteShader", "ps_5_1", &m_pd3dPixelShaderBlob));
}

D3D12_SHADER_BYTECODE DepthRenderShader::CreateVertexShader(int pipeline)
{
	if (pipeline == 0) {
		return(Shader::CompileShaderFromFile(L"Shaders.hlsl", "VSStandard", "vs_5_1", &m_pd3dVertexShaderBlob));
	}
...
D3D12_DEPTH_STENCIL_DESC DepthRenderShader::CreateDepthStencilState()
{
	D3D12_DEPTH_STENCIL_DESC d3dDepthStencilDesc;
	::ZeroMemory(&d3dDepthStencilDesc, sizeof(D3D12_DEPTH_STENCIL_DESC));
	d3dDepthStencilDesc.DepthEnable = TRUE;
...
D3D12_RASTERIZER_DESC DepthRenderShader::CreateRasterizerState()
{
	D3D12_RASTERIZER_DESC d3dRasterizerDesc;
	::ZeroMemory(&d3dRasterizerDesc, sizeof(D3D12_RASTERIZER_DESC));
	d3dRasterizerDesc.FillMode = D3D12_FILL_MODE_SOLID;
	...
	d3dRasterizerDesc.DepthBias = 7000; // 250000
#endif
	d3dRasterizerDesc.DepthBiasClamp = 0.0f;
...
D3D12_INPUT_LAYOUT_DESC DepthRenderShader::CreateInputLayout(int pipeline)
{
	if (pipeline == 0) {
		UINT nInputElementDescs = 5;
		D3D12_INPUT_ELEMENT_DESC* pd3dInputElementDescs = new D3D12_INPUT_ELEMENT_DESC[nInputElementDescs];
...
void DepthRenderShader::BuildObjects(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, void* pContext)
{
...
void DepthRenderShader::ReleaseObjects()
{
	for (int i = 0; i < MAX_DEPTH_TEXTURES; i++)
	{
...
void DepthRenderShader::CreateShaderVariables(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList)
{
...
void DepthRenderShader::UpdateShaderVariables(ID3D12GraphicsCommandList* pd3dCommandList)
{
	...
	pd3dCommandList->SetGraphicsRootConstantBufferView(19, d3dcbToLightGpuVirtualAddress); //ToLight
}

void DepthRenderShader::ReleaseShaderVariables()
{
...
void DepthRenderShader::CreateShader(ID3D12Device* pd3dDevice, ID3D12RootSignature* pd3dGraphicsRootSignature, UINT nRenderTargets, DXGI_FORMAT* pdxgiRtvFormats, DXGI_FORMAT dxgiDsvFormat)
{
	m_nPipelineStates = 2;
	...
	m_d3dPipelineStateDesc.BlendState = CreateBlendState();
	m_d3dPipelineStateDesc.DepthStencilState = CreateDepthStencilState();
	m_d3dPipelineStateDesc.InputLayout = CreateInputLayout(1);
	m_d3dPipelineStateDesc.SampleMask = UINT_MAX;
	m_d3dPipelineStateDesc.PrimitiveTopologyType = D3D12_PRIMITIVE_TOPOLOGY_TYPE_TRIANGLE;
	m_d3dPipelineStateDesc.NumRenderTargets = nRenderTargets;
	for (UINT i = 0; i < nRenderTargets; i++) m_d3dPipelineStateDesc.RTVFormats[i] = (pdxgiRtvFormats) ? pdxgiRtvFormats[i] : DXGI_FORMAT_R8G8B8A8_UNORM;
...
void DepthRenderShader::PrepareShadowMap(ID3D12GraphicsCommandList* pd3dCommandList)
{
	for (int j = 0; j < MAX_LIGHTS; j++)
	{
		if (m_pLights[j].m_bEnable)
		{
			...
			float fNearPlaneDistance = 5.0f, fFarPlaneDistance = m_pLights[j].m_fRange;
			...
			m_ppDepthRenderCameras[j]->SetLookAt(m_pLights[j].m_xmf3Direction);
...
void DepthRenderShader::Render(ID3D12GraphicsCommandList* pd3dCommandList, Camera* pCamera)
{
	if (m_ppd3dPipelineStates && m_ppd3dPipelineStates[0]) pd3dCommandList->SetPipelineState(m_ppd3dPipelineStates[0]);

	pCamera->SetViewportsAndScissorRects(pd3dCommandList);
	pCamera->UpdateShaderVariables(pd3dCommandList);

	if (m_pScene) m_pScene->Depthrender(pd3dCommandList, 0);

	if (m_ppd3dPipelineStates && m_ppd3dPipelineStates[1]) pd3dCommandList->SetPipelineState(m_ppd3dPipelineStates[1]);

	if (m_pScene) m_pScene->Depthrender(pd3dCommandList, 1);
}

//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
ShadowMapShader::ShadowMapShader(GameScene* Scene)
{
	m_pScene = Scene;
}
...
D3D12_SHADER_BYTECODE ShadowMapShader::CreateVertexShader()
{
	return(Shader::CompileShaderFromFile(L"Shaders.hlsl", "VSShadowMapShadow", "vs_5_1", &m_pd3dVertexShaderBlob));
}

D3D12_SHADER_BYTECODE ShadowMapShader::CreatePixelShader()
{
	return(Shader::CompileShaderFromFile(L"Shaders.hlsl", "PSShadowMapShadow", "ps_5_1", &m_pd3dPixelShaderBlob));
}

void ShadowMapShader::CreateShaderVariables(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList)
{
}

void ShadowMapShader::ReleaseShaderVariables()
{
}

void ShadowMapShader::UpdateShaderVariables(ID3D12GraphicsCommandList* pd3dCommandList)
{
	if (m_pDepthTexture) m_pDepthTexture->UpdateShaderVariables(pd3dCommandList);
}

void ShadowMapShader::ReleaseUploadBuffers()
{
}

void ShadowMapShader::BuildObjects(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, void* pContext)
{
...
void ShadowMapShader::ReleaseObjects()
{
	if (m_pDepthTexture) m_pDepthTexture->Release();
}

void ShadowMapShader::Render(ID3D12GraphicsCommandList* pd3dCommandList)
{
	Shader::Render(pd3dCommandList);

	UpdateShaderVariables(pd3dCommandList);
	//m_pScene->m_pPlayer->m_pCamera->update(pd3dCommandList);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
TextureToViewportShader::TextureToViewportShader()
{
}
...
D3D12_DEPTH_STENCIL_DESC TextureToViewportShader::CreateDepthStencilState()
{
	D3D12_DEPTH_STENCIL_DESC d3dDepthStencilDesc;
	::ZeroMemory(&d3dDepthStencilDesc, sizeof(D3D12_DEPTH_STENCIL_DESC));
	d3dDepthStencilDesc.DepthEnable = FALSE;
...
void TextureToViewportShader::BuildObjects(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, void* pContext)
{
}

void TextureToViewportShader::ReleaseObjects()
{
}

void TextureToViewportShader::Render(ID3D12GraphicsCommandList* pd3dCommandList)
{
	Shader::Render(pd3dCommandList);
...
D3D12_SHADER_BYTECODE ParticleShader::CreateVertexShader(int nPipelineState)
{
	if (nPipelineState == 0)
		return(Shader::CompileShaderFromFile(L"Shaders.hlsl", "VSParticleStreamOutput", "vs_5_1", &m_pd3dVertexShaderBlob));
	else
		return(Shader::CompileShaderFromFile(L"Shaders.hlsl", "VSParticleDraw", "vs_5_1", &m_pd3dVertexShaderBlob));
}

D3D12_SHADER_BYTECODE ParticleShader::CreateGeometryShader(int nPipelineState)
{
	if (nPipelineState == 0)
		return(Shader::CompileShaderFromFile(L"Shaders.hlsl", "GSParticleStreamOutput", "gs_5_1", &m_pd3dGeometryShaderBlob));
	else
		return(Shader::CompileShaderFromFile(L"Shaders.hlsl", "GSParticleDraw", "gs_5_1", &m_pd3dGeometryShaderBlob));
}

D3D12_SHADER_BYTECODE ParticleShader::CreatePixelShader(int nPipelineState)
{
	if (nPipelineState == 0)
		return(Shader::CompileShaderFromFile(L"Shaders.hlsl", "PSParticleDraw", "ps_5_1", &m_pd3dPixelShaderBlob));
	else
		return(Shader::CompileShaderFromFile(L"Shaders.hlsl", "PSParticleDraw", "ps_5_1", &m_pd3dPixelShaderBlob));
}
...
void ParticleShader::CreateShader(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature, int nPipelineState)
{
	...
	m_d3dPipelineStateDesc.pRootSignature = pd3dGraphicsRootSignature;
	m_d3dPipelineStateDesc.VS = CreateVertexShader(0);
	m_d3dPipelineStateDesc.GS = CreateGeometryShader(0);
	m_d3dPipelineStateDesc.PS = CreatePixelShader(0);
	m_d3dPipelineStateDesc.StreamOutput = CreateStreamOuputState(0);
	m_d3dPipelineStateDesc.RasterizerState = CreateRasterizerState(0);
	m_d3dPipelineStateDesc.BlendState = CreateBlendState(0);
	m_d3dPipelineStateDesc.DepthStencilState = CreateDepthStencilState(0);
	m_d3dPipelineStateDesc.InputLayout = CreateInputLayout(0);
	m_d3dPipelineStateDesc.SampleMask = UINT_MAX;
	m_d3dPipelineStateDesc.PrimitiveTopologyType = GetPrimitiveTopologyType(0);
	m_d3dPipelineStateDesc.NumRenderTargets = GetNumRenderTargets(0);
	m_d3dPipelineStateDesc.RTVFormats[0] = GetRTVFormat(0, 0);
	m_d3dPipelineStateDesc.DSVFormat = GetDSVFormat(0);
	m_d3dPipelineStateDesc.SampleDesc.Count = 1;
	m_d3dPipelineStateDesc.Flags = D3D12_PIPELINE_STATE_FLAG_NONE;
	...
	m_d3dPipelineStateDesc.pRootSignature = pd3dGraphicsRootSignature;
	m_d3dPipelineStateDesc.VS = CreateVertexShader(1);
	m_d3dPipelineStateDesc.GS = CreateGeometryShader(1);
	m_d3dPipelineStateDesc.PS = CreatePixelShader(1);
	m_d3dPipelineStateDesc.StreamOutput = CreateStreamOuputState(1);
	m_d3dPipelineStateDesc.RasterizerState = CreateRasterizerState(1);
	m_d3dPipelineStateDesc.BlendState = CreateBlendState(1);
	m_d3dPipelineStateDesc.DepthStencilState = CreateDepthStencilState(1);
	m_d3dPipelineStateDesc.InputLayout = CreateInputLayout(1);
	m_d3dPipelineStateDesc.SampleMask = UINT_MAX;
	m_d3dPipelineStateDesc.PrimitiveTopologyType = GetPrimitiveTopologyType(1);
	m_d3dPipelineStateDesc.NumRenderTargets = GetNumRenderTargets(1);
	m_d3dPipelineStateDesc.RTVFormats[0] = GetRTVFormat(1, 0);
	m_d3dPipelineStateDesc.DSVFormat = GetDSVFormat(1);
	m_d3dPipelineStateDesc.SampleDesc.Count = 1;
	m_d3dPipelineStateDesc.Flags = D3D12_PIPELINE_STATE_FLAG_NONE;
...
void ParticleShader::OnPrepareRender(ID3D12GraphicsCommandList* pd3dCommandList, int nPipelineState)
{
	if (m_pd3dCbvSrvDescriptorHeap) pd3dCommandList->SetDescriptorHeaps(1, &m_pd3dCbvSrvDescriptorHeap);
