...
DepthRenderShader::DepthRenderShader(GameScene* pScene, LIGHT* pLights)
{
	m_pScene = pScene;
...
void DepthRenderShader::UpdateShaderVariables(ID3D12GraphicsCommandList* pd3dCommandList)
{
	...
	pd3dCommandList->SetGraphicsRootConstantBufferView(20, d3dcbToLightGpuVirtualAddress); //ToLight
}
...
void DepthRenderShader::BuildObjects(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, void* pContext)
{
	...
	d3dDescriptorHeapDesc.NumDescriptors = MAX_LIGHTS;
	...
	m_pDepthTexture = new Texture(MAX_LIGHTS, RESOURCE_TEXTURE2D_ARRAY, 0, 1);

	D3D12_CLEAR_VALUE d3dClearValue = { DXGI_FORMAT_R32_FLOAT, { 1.0f, 1.0f, 1.0f, 1.0f } };
	for (UINT i = 0; i < MAX_LIGHTS; i++) m_pDepthTexture->CreateTexture(pd3dDevice, _DEPTH_BUFFER_WIDTH, _DEPTH_BUFFER_HEIGHT, DXGI_FORMAT_R32_FLOAT, D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET, D3D12_RESOURCE_STATE_COMMON, &d3dClearValue, RESOURCE_TEXTURE2D, i);
		...
		if (m_pLights) {
			if (m_pLights[j].m_bEnable)
			{
				...
				if (m_pLights[j].m_nType == DIRECTIONAL_LIGHT)
				{
					...
					xmmtxProjection = XMMatrixOrthographicOffCenterLH(fLeft * 6.0f, fRight * 6.0f, fBottom * 6.0f, fTop * 6.0f, fNearPlaneDistance, fFarPlaneDistance);
				}
...
ShadowMapShader::ShadowMapShader(GameScene* pScene)
{
	m_pScene = pScene;
}
...
void ShadowMapShader::Render(ID3D12GraphicsCommandList* pd3dCommandList)
{
	...
	//if (m_pScene) m_pScene->defrender(pd3dCommandList);
}
